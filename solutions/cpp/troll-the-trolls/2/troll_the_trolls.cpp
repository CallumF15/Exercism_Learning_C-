...
// should return a `bool`.
    ...
    {
        if (action == Action::read) //everyone can read
            return true;
        
        if (action == Action::remove && accountStatus == AccountStatus::mod) //only mods can remove
            return true;

        if (action == Action::write && accountStatus != AccountStatus::guest) //everyone but guest can write
            return true;
...
// account has a strictly higher priority than the second.
    ...
    bool has_priority(AccountStatus actor, AccountStatus target)
    {
         return actor > target; //works since enum evaluate to numbers (0,1,2,4)
    }
}  // namespace hellmath
